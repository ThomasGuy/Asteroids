...
import sys
from game import game_state, helper, explosion, asteroid
import pyglet
...
game = game_state.State(game_window)
score_label = Label(text="Score: 0", x=10, y=HEIGHT - 15, batch=main_batch)
level_label = Label(text="Pyglet Asteroids2 level 1", font_size=18, x=WIDTH / 2,
                    y=HEIGHT - 15, anchor_x='center', batch=main_batch)
# Set up the game over label off screen
game_over_label = Label(text="GAME OVER", x=WIDTH / 2, y=-300, anchor_x='center',
...
def update(dt):
    ...
    missiles, rocks, hits = helper.group_group_collide(game.get_missiles(),
                                                       game.get_rocks())
    # Missiles and enemy ships
    bullets, enemy, kills = helper.group_group_collide(missiles,
                                                       game.get_enemy_ship())
    ...
    if not game_over:
        ...
        pebbles, collision = helper.group_collide(rocks, game.get_player_ship())
        game.reset_rocks(pebbles)
        # Player_ship dies by enemy missiles
        enemy_missiles, kill = helper.group_collide(game.get_enemy_missiles(),
    ...
    for to_remove in [obj for obj in game.get_game_objects() if obj.dead]:
        ...
        if isinstance(to_remove, asteroid.Asteroid):
            explosion.Explosion(x=to_remove.x, y=to_remove.y, batch=main_batch)
    ...
    # Update game objects
    game.update_game_objects(Add_Game_Objects | game.get_missiles()
                             | game.get_enemy_missiles() | game.get_enemy_ship())
