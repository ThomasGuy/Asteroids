...
from . import resources, asteroid, helper, player, enemy_ships
import random
...
class State:
    ...
    def mk_player_ship(self, x, y, batch):
        self.player_ship = player.Player(x=x, y=y, batch=batch,
    ...
    def mk_enemy_ship(self, batch):
        ...
        enemy_ship = enemy_ships.EnemyShips(x=enemy_x, y=enemy_y, batch=batch,
                                            dimension=self.win_size)
    ...
    def spawn_rocks(self, batch=None):
        ...
        while count < self._num_rocks:
            asteroid_pos = (random.randint(0, self.win_size[0]),
                            random.randint(0, self.win_size[1]))
            if helper.distance(self.player_ship.position, asteroid_pos) > 200:
                rock = random.choice(resources.roids)
                new_asteroid = asteroid.Asteroid(img=resources.get_image(rock),
